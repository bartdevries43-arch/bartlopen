...
await page.goto(`file://${path.join(here, 'hrv-carousel.html')}`, { waitUntil: 'networkidle' });
await page.evaluate(() => document.fonts.ready);
const slides = page.locator('.slide');
if (await slides.count() !== 8) throw new Error('HRV-carousel moet 8 slides hebben');
for (let i = 0; i < 8; i += 1) {
  await slides.nth(i).screenshot({
    path: path.join(out, `hrv-pro-${String(i + 1).padStart(2, '0')}.png`),
    type: 'png'
  });
}
await browser.close();
console.log('HRV PRO-carousel gerenderd');
