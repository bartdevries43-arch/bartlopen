...
await page.goto(`file://${path.join(here, 'easy-carousel.html')}`, { waitUntil: 'networkidle' });
const slides = page.locator('.slide');
for (let i = 0; i < await slides.count(); i++) {
  await slides.nth(i).screenshot({ path: path.join(out, `easy-pro-${String(i + 1).padStart(2, '0')}.png`), type: 'png' });
}
await browser.close();
console.log('Rustige duurloop PRO-carousel gerenderd');
